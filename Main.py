...
class Main(object):
    ...
    def verificador_endereco(_self, endereco, _mapa):
        ...
        if (fronteira):
            try:
                gmaps = googlemaps.Client(key='AIzaSyCtUJa2dQX1s6Nbqv5hv-Rxbk7AKAKMYCQ')
    ...
    def exibir(self):
        ...
        if (len(st.session_state['ENDERECO']) > 5):
            ...
            if (geometria == True and zona == True):
                st.sidebar.caption('Endereço já pesquisado')
            elif (geometria == None):
                st.sidebar.caption('Não foi possível encontrar o endereço na cidade de Cachoeirinha - RS')
            elif (zona == None):
                st.sidebar.caption('Não foi possível identificar a zona do endereço especificado')
            else:
                dt_final = pd.DataFrame({'ENDERECO': [st.session_state['ENDERECO']]})
                dt_final.loc[0, ['ZONA', 'GEOMETRIA']] = [zona, geometria]
                st.sidebar.caption(f"O endereço \"{st.session_state['ENDERECO']}\" pertence a {zona}")
